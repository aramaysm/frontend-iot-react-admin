...
*/
  ...
  static InverseOf(matrix: Elementary_Matrix): Square_Hill_Matrix {
    ...
    let result: Square_Hill_Matrix = new Square_Hill_Matrix(order,[]);
      ...
      let rFactor: Modular_Data = new Modular_Data(0);
      ...
      let rV1Data: Array<Modular_Data> = new Array<Modular_Data>(order);
      let rV2Data: Array<Modular_Data> = new Array<Modular_Data>(order);
      ...
      for (let row = 0; row < order; row++)
        for (let col = 0; col < order; col++) {
          ...
          result._matrix[row * order + col] = Modular_Data.operator_sub(
            iMatItem,Modular_Data.operator_mult(factor,
              Modular_Data.operator_mult(
